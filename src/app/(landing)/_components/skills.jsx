"use client";
import React from "react";
import { motion, useTransform, useScroll } from "framer-motion";

import Wrapper from "./wrapper";
import SkillIcon from "./skill-icon";
import Div from "./div";

// IMAGES

import Service from "./service";

const Skills = () => {
    const { scrollY } = useScroll();
    const y1 = useTransform(scrollY, [0, 1500], [1000, 0], { clamp: false });
    const y2 = useTransform(scrollY, [0, 2200], [1500, 0], { clamp: false });
    const y3 = useTransform(scrollY, [0, 2700], [2000, 0], { clamp: false });
    return (
        <div
            id="skills"
            className="bg-[#111111] py-[50px] md:py-[100px] relative overflow-hidden"
        >
            {/* BACKGROUND ELEMENTS START */}
            <div className="sec-3-bg-gradient-1" />
            <div className="sec-3-bg-gradient-2" />
            <motion.img
                className="sec-3-p-e-1"
                style={{ y: y1 }}
                src="/sec-3-p-e-1.png"
            />
            <motion.img
                className="sec-3-p-e-2"
                style={{ y: y2 }}
                src="/sec-3-p-3-e-2.png"
            />
            <motion.img
                className="sec-3-p-e-3"
                style={{ y: y3 }}
                src="/sec-3-p-e-3.png"
            />
            {/* BACKGROUND ELEMENTS END */}
            <Wrapper>
                {/* SKILL ICONS START */}
                <Div className="grid grid-cols-4 gap-3 md:grid-cols-11 relative">
                    <SkillIcon path="/gitt.png" />
                    <SkillIcon path="/react.png" />
                    <SkillIcon path="/html.png" />
                    <SkillIcon path="/figma.png" />
                    <SkillIcon path="/css.png" />
                    <SkillIcon path="/js.png" />
                    <SkillIcon path="/next.svg" />
                    <SkillIcon path="/vercel.svg" />
                </Div>
                {/* SKILL ICONS END */}

                {/* SERVICES SECTION START */}
                <div className="flex items-end justify-between flex-wrap-reverse py-[50px] md:py-[100px] relative">
                    {/* SERVICES START */}
                    <div className="md:max-w-[65%] flex flex-col gap-6 md:gap-10">
                        <Service
                            num="1"
                            title="Front-End Development"
                            desc="I build fast, responsive, and accessible websites using modern technologies like React, Next.js, and Tailwind CSS. My focus is on performance, usability, and clean code."
                            data={[
                                "React",
                                "Next.js",
                                "Tailwind CSS",
                                "Responsive Design",
                                "Performance Optimization",
                            ]}
                        />
                        <Service
                            num="2"
                            title="UI Implementation"
                            desc="I turn design files into fully functional interfaces. From Figma to code — I translate visual designs into seamless user experiences."
                            data={[
                                "Figma to Code",
                                "Pixel-perfect Layouts",
                                "Component-based Structure",
                                "Accessibility (a11y)",
                                "Clean & Scalable Code",
                            ]}
                        />
                        <Service
                            num="3"
                            title="Dynamic Web Features"
                            desc="I develop interactive features such as search, archiving, and local data handling using JavaScript and browser APIs."
                            data={[
                                "Search Functionality",
                                "Archive System",
                                "LocalStorage Integration",
                                "State Management",
                                "Custom Hooks",
                            ]}
                        />
                        <Service
                            num="4"
                            title="Portfolio & Landing Pages"
                            desc="I create minimal, modern and user-focused portfolios or landing pages that help individuals and brands express their identity online."
                            data={[
                                "Modern UI",
                                "Mobile-First Design",
                                "Brand-Focused Layouts",
                                "Custom Animations",
                                "SEO Basics",
                            ]}
                        />
                    </div>

                    {/* SERVICES END */}

                    {/* SECTION HEADING START */}
                    <div className="flex flex-wrap justify-center gap-2 md:gap-0 md:flex-col text-[40px] md:text-[90px] 2xl:text-[120px] leading-[40px] md:leading-[95px] 2xl:leading-[123px] font-oswald uppercase text-gradient mb-10 md:mb-0 md:sticky top-2">
                        <span>Solving</span>
                        <span>Problems</span>
                        <span className="flex items-center gap-2">
                            Through
                            <img
                                src="/heading-pattern.png"
                                className="block md:hidden w-[80px] mt-1"
                            />
                        </span>
                        <span className="flex items-center gap-4">
                            Good{" "}
                            <img
                                src="/heading-pattern.png"
                                className="hidden md:block md:w-[140px] 2xl:w-[190px] mt-3"
                            />
                        </span>
                        <span>Services</span>
                    </div>
                    {/* SECTION HEADING END */}
                </div>
                {/* SERVICES SECTION END */}
            </Wrapper>
        </div>
    );
};

export default Skills;
